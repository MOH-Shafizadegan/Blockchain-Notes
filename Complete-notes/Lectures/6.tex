...
$C^{⌊^{k}} \leq C_{2}$,
where $C_{1} ≡ C_{i_{1}}^{h^{1}}$ \text{and} $C_{1} ≡ C_{i_{2}}^{h^{2}}.$\\
...
\begin{equation*}
	\frac{1}{(1 - \beta) \lambda} \ge \frac{1}{\beta\lambda}.
\end{equation*}
...
So the probability of deconfirmation decays exponentially in k, as also illustrated in Figure \ref{fig:L6_f2}. Turns out this calculation was also conducted by Nakamoto themselves, albeit somewhat incorrectly, which is included in Figure 2 as a comparison.
\begin{figure}[h!]
	\centering
